...
export const FeaturedWork: React.FC<FeaturedWorkProps> = ({ projects, onSelectProject }) => {
  // Highlight top 3 projects: Velora, Taskflow, Finmate
  const featured = projects.filter((p) => ['velora', 'taskflow', 'finmate'].includes(p.id));

  return (
    <section className="py-24 border-t border-slate-800/80 bg-[#06080D]">
      <div className="container-vortex">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 pb-6 border-b border-slate-800">
          <div>
            <div className="text-xs font-mono text-blue-400 tracking-widest uppercase mb-2 flex items-center gap-2">
        ...
        <div className="space-y-24">
          {featured.map((project, index) => {
            ...
            return (
              <div
                key={project.id}
                className="group relative rounded-3xl border border-slate-800 bg-[#0A0E18] overflow-hidden hover:border-slate-700 transition-all duration-500 shadow-2xl"
              >
                <div className={`grid grid-cols-1 lg:grid-cols-12 gap-8 items-center ${isReversed ? 'lg:flex-row-reverse' : ''}`}>
                  ...
                  <div className={`lg:col-span-5 p-8 sm:p-12 space-y-6 ${isReversed ? 'lg:order-2' : 'lg:order-1'}`}>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="font-display font-bold text-4xl sm:text-5xl text-slate-700 group-hover:text-blue-500/50 transition-colors">
                    ...
                    {/* Category & Title */}
                    <div>
                      <div className="text-xs font-mono tracking-wider text-blue-400 uppercase font-semibold mb-1">
                        {project.category}
                      </div>
                      <h3 className="font-display font-bold text-2xl sm:text-4xl text-white tracking-tight group-hover:text-blue-300 transition-colors">
                        {project.name}
                      </h3>
                    </div>
                    ...
                    {/* Highlight Box */}
                    <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800/80 text-xs text-slate-300 font-body leading-relaxed">
                      <strong className="text-white block font-mono text-[11px] uppercase tracking-wider mb-1 text-slate-400">
                  ...
                  <div className={`lg:col-span-7 p-4 sm:p-6 lg:p-8 ${isReversed ? 'lg:order-1' : 'lg:order-2'}`}>
                    <div
                      onClick={() => onSelectProject(project)}
                      data-cursor="INSPECT"
                      className="relative rounded-2xl overflow-hidden border border-slate-800/90 bg-slate-950 cursor-pointer group-hover:border-blue-500/30 transition-all duration-500 shadow-inner group/img"
                    >
                      ...
                      <div className="relative aspect-[16/10] overflow-hidden bg-slate-950">
                        ...
                        <div className="absolute inset-0 bg-blue-950/20 opacity-0 group-hover/img:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-[1px]">
                          <span className="px-5 py-2.5 rounded-full bg-blue-600/90 text-white font-mono text-xs font-semibold tracking-wider shadow-lg flex items-center gap-2 transform translate-y-2 group-hover/img:translate-y-0 transition-transform">
